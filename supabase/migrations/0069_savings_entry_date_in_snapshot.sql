-- 0069_savings_entry_date_in_snapshot.sql
-- The Tích lũy widget shows a "saved this month" momentum spark, which needs to
-- attribute each savings_entry to a month. entry_date has existed since 0010 but
-- was never shipped in the snapshot. This adds it to the savings_entries select
-- (purely additive; every other field of get_family_snapshot is verbatim from the
-- live definition, so CREATE OR REPLACE leaves the rest untouched). entry_date is
-- plaintext (a date, never encrypted), so no enc handling changes.
