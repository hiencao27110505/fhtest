-- 0131 — display-grade money source on family transactions (txn-listing revamp Q18b).
--
-- `instrument` is a captured STRING ("VIB · tín dụng ••4512"), written once at
-- email-import promotion and shown/filtered on the Giao dịch screen. It is
-- deliberately not a foreign key: family-level accounts/balances do not exist
-- (that is its own epic — personal_accounts stays per-user). Old rows simply
-- have null. Snapshot RPC re-created with the one extra column.

alter table public.transactions add column if not exists instrument text;
