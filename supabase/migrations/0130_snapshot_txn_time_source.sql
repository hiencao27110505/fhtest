-- 0130 — get_family_snapshot: transactions gain occurred_time / occurred_time_enc /
-- created_at / source.
--
-- Why: 0096 added the time columns and 0100 added `source`, but the snapshot RPC
-- (last touched in 0069) never learned them — so on the RPC hydrate path family
-- rows arrived with no time (a latent gap: times only survived until the next
-- full hydrate), no created_at (hydrate maps it to _ts for same-day ordering),
-- and no provenance. The Giao dịch screen's "Nguồn ghi" filter (txn-listing
-- revamp) needs `source`; the other two ride along to close the gap.
--
-- Only the transactions subselect changes; everything else is verbatim the live
-- definition (pulled 2026-09-12).
    ...
    'transactions', (
      select coalesce(json_agg(row_to_json(t) order by t.txn_date desc), '[]'::json) from (
        select id, category_id, member_id, note, note_enc, amount, amount_enc,
               occurred_time, occurred_time_enc, txn_date, status, created_by, created_at, source
        from transactions
