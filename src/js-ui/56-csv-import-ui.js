...
var FH_TXH_VIEWERS = ['gichisreading@gmail.com'];
function csvTxrViewerOk(){
  var em = ((window.fhUser && window.fhUser.email)
    || (window.FAM && window.FAM.user && window.FAM.user.email) || '').toLowerCase();
  return FH_TXH_VIEWERS.indexOf(em) !== -1;
}

/* The whole header, synced from renderCsvReview. Clears itself outside staged
   mode — and for anyone not on the viewer allowlist above — so the file
   importer and non-viewers never see any of this. */
function csvTxrHeadSync(){
  var head = document.getElementById('txh'); if(!head) return;
  if(!csvStagedMode || !csvReview || !csvTxrViewerOk()){ head.innerHTML = ''; csvTxrKey = null; return; }
  if(!head.firstChild){
