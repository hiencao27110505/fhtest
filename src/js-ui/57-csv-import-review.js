...
/* ═══ ONE account identity ═══════════════════════════════════════════════════
   ...
   signal counterpart, the statement path). The alias, long-name, display and
   deny tables that used to sit here moved into taxonomy/providers.json
   (account-identity-spec P6): one registry, generated by tools/gen-providers.js
   into this runtime (09-providers.js, loaded before this file) and into the
   worker's providers.mjs, so device and server cannot drift. These wrappers
   stay as thin lookups so no call site churns.

   A known provider keys as its permanent registry key — "Ví MoMo", "M_Service"
   and the operator's legal name are all `momo`. Prose the registry has never
   seen folds by the same shape rules it always did (the corporate dressing,
   a trailing "Việt Nam") into a stable slug, which can never collide with a
   registry key and so matches only its own spelling class. Both fallback
   tries run through csvCanonicalProvider first, because the old fold always
   did and the registry's own squash does not know its channel-noise strip:
   without it, "VCB Digibank" would miss the key and "Đông Á" / "Dong A Bank"
   would slug apart. Returns '' for a phrase that is not a provider at all
   (the deny list — "Ngân hàng liên kết", "Tài khoản" — rode into the
   registry too). */
function fhAcctProviderKey(name){
  var R = window.FH_PROVIDERS, noiseless = csvCanonicalProvider(name);
  return R.keyOf(name) || R.keyOf(noiseless) || R.slugOf(noiseless);
}
...
function fhProviderName(name){
  ...
  /* The same fold the identity key uses, so "Ví MoMo" and the operator's legal
     name read as the one registry spelling instead of as themselves. A fold
     that ends in a slug names a provider the registry does not know: it reads
     as printed, never folded into another. */
  var key = fhAcctProviderKey(name);
  return (key && window.FH_PROVIDERS.labelOf(key)) || String(name).trim();
}
