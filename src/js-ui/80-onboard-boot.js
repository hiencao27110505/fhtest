...
/* The default categories a brand-new family gets. MUST mirror
   seed_default_categories() (latest: migration 0055) in the SAME ORDER, names,
   emojis and colors — the server seeds these at create_family, the onboarding
   budget step maps its per-category amounts back onto them by sort_order, and the
...
/* Budget setup step (create only). Categories are already seeded (obSeedCats);
   ...
   total using the app's own weights (CATW, 20-budget.js). "Khác/Others" is a real,
   always-present row, but it's the BALANCER: read-only, showing total − sum(the
   rest), floored at 0. The named rows are clamped so they can never out-allocate
   the total, so the categories ALWAYS add up to exactly the monthly budget.
   Everything is optional — Continue with an empty budget (or "Set up later"). */
function obPrefillBudget(){
  ...
  // Others — the read-only catch-all that soaks up whatever's left, so the sum stays exact
  var fs=catStyle[CAT_FALLBACK]||['🗂️','#eef4fb','var(--cat-other)'];
  rows += '<div class="ob-catbud ob-catbud-fb"><span class="ob-catbud-ic" style="background:'+fs[1]+';color:'+fs[2]+'">'+fs[0]+'</span>'
    +'<span class="ob-catbud-n">'+esc(L('Khác','Others'))+' <span class="ob-catbud-tag">'+L('còn lại','left')+'</span></span>'
    +'<span class="ob-catbud-in ob-catbud-ro">'+sym+'<span class="ob-others-val" id="ob-others-val">0</span></span></div>';
  body.innerHTML =
...
function obBudgetTotal(){ var el=document.getElementById('ob-budget'); return el?(parseAmtBase(el.value)||0):0; }
function obCatInputs(){ return [].slice.call(document.querySelectorAll('#ob-catbudgets .ob-cat-bud')); }
function obNamedSum(except){ var s=0; obCatInputs().forEach(function(i){ if(i!==except) s+=parseAmtBase(i.value)||0; }); return s; }
/* Repaint the read-only "Others" balancer = total − sum(named), floored at 0. */
function obSyncOthers(){
  var ov=document.getElementById('ob-others-val'); if(!ov) return;
  var left=Math.max(0, obBudgetTotal()-obNamedSum(null));
  ov.textContent=left?amtToInput(left):'0';
}
/* A named category was edited: clamp it so the named rows can never sum past the
   total (Others can't go negative), then repaint Others. The sum is always exact. */
function obCatEdit(inp){
  inp.dataset.touched='1';
  var total=obBudgetTotal();
  if(total>0){
    var room=Math.max(0, total-obNamedSum(inp));      // what's free once the OTHER named rows are counted
    if((parseAmtBase(inp.value)||0)>room) inp.value = room?amtToInput(room):'';
  }
  obSyncOthers();
}
/* Total changed: re-suggest the untouched rows from the new total, then — if the
   hand-set rows now overflow it — scale every named row down to fit, so the sum is
   still exactly the total (with Others at 0). */
function obTotalChange(){
  obSuggestBudgets();
  var total=obBudgetTotal(), used=obNamedSum(null);
  if(total>0 && used>total){
    var scale=total/used;
    obCatInputs().forEach(function(i){ var v=Math.round((parseAmtBase(i.value)||0)*scale/10)*10; i.value=v?amtToInput(v):''; });
  }
  obSyncOthers();
}
/* Auto-split the total across untouched named rows by the app's CATW weights,
   reserving a slice for Others so it starts with a sensible default too. */
function obSuggestBudgets(){
  var total=obBudgetTotal(); if(!total){ obSyncOthers(); return; }
  var inputs=obCatInputs();
  var free=inputs.filter(function(i){ return i.dataset.touched!=='1'; });
  if(!free.length){ obSyncOthers(); return; }
  ...
  var OTHERS_W=0.10;                                   // Others keeps ~10% by default
  ...
  var sum=ws.reduce(function(a,b){return a+b;},0)+OTHERS_W;
  free.forEach(function(i,k){ var base=Math.round(pool*(ws[k]/sum)/10)*10; i.value=base?amtToInput(base):''; });
  obSyncOthers();
}
function obBudgetNext(){
  obTotalChange();                                    // final reconcile: named rows are clamped/scaled to the total
  FAM.budget=obBudgetTotal();
  if(FAM.budget>0){
    FAM.catBudget={};
    obCatInputs().forEach(function(inp){ var c=inp.getAttribute('data-cat'), v=parseAmtBase(inp.value)||0; if(v>0) FAM.catBudget[c]=v; });
  } else {
    FAM.catBudget=null;                               // no total → don't persist orphan category budgets
  }
  finishOnboarding();
}
