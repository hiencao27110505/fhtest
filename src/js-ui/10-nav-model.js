...
function go(name){
  // Personal-first: with no family yet (fh-nofam) the tabbar is hidden and every
  // family tab — Home included — funnels to Tài chính, the famless account's only
  // home (deep links / push routes can still call go with them).
  if((name==='spending'||name==='events'||name==='home') && document.documentElement.classList.contains('fh-nofam')) name='personal';
  ...
  if(name==='personal'){ try{ if(typeof renderPersonal==='function') renderPersonal(); if(window.fhPersonalData && fhPersonalData().state==='boot' && window.fhPersonalBoot) fhPersonalBoot(); }catch(e){}
    ...
    // Not while the tab already shows the queue as a deck (state 2, or the
    // standalone widget whenever rows wait): two surfaces would compete for
    // the same tap. A notification tap still forces the sheet.
    try{ if(window.fhQuickReviewMaybe) setTimeout(function(){ if((window.persActState && persActState()===2) || (window.fhStagedCount||0)>0) return; window.fhQuickReviewMaybe(); }, 450); }catch(e){}
  }
}
