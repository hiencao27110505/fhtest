...
function renderTxnStats(list){
  ...
  /* Bars are scaled to the tallest bar IN VIEW, not the whole range — the
     initial global-max heights are only a first paint; txnStripScale() takes
     over immediately and again on every scroll, so what you're looking at
     always uses the full height. data-out carries each bucket's value. */
  var cols=keys.map(function(k){
    ...
    return '<button type="button" class="pst-c" data-out="'+(outs[k]||0)+'" onclick="txnBarTap(&#39;'+k+'&#39;)" aria-label="'+escAttr(_txStripLbl(k))+'">'
...
/* A bar tap only PINS its value now — it never scrolls the list. Repaint just
   the stat card so the list (and the strip's scroll position) stay put. */
function txnBarTap(k){
  TXV.pin=(TXV.pin===k)?null:k;
  renderTxnStats(TXV._list||[]);
}
