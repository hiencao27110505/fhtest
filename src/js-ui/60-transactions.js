...
function txRow(t){
  ...
  var whoD=(t.who==='Shared'||t.who==='Both')?L('Chung',t.who):t.who;
  return '<div class="row tap" onclick="openEditExpense(\''+t.id+'\')"><div class="r-ico-wrap"><div class="r-ico" style="background:'+s[1]+';color:'+s[2]+'">'+t.ico+'</div>'+spAv(t.who)+'</div>'
    +'<div class="r-body"><div class="r-t">'+t.note+'</div><div class="r-s">'+dstr+' · '+t.cat+' · '+whoD+'</div></div>'
...
function renderTxnScreen(){
  ...
  var total=list.reduce(function(s,t){return s+(t.amt||0);},0);
  setHTML('txn-sum','<b>'+list.length+'</b> giao dịch · tổng <span class="num">'+fmt(total)+'</span>');
  var html='';
  if(!list.length){
