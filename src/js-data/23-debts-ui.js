  ...
  (function () {
    ...
    /* "Cài đặt tài khoản" is the account screen in settings mode — see
       fhAcctEditSheet in the Account setup block below (one screen, two modes). */
    ...
    const _wzLive = (acct) => {
      ...
      if (acct.kind === 'credit_card') { const ce = (fhPersonalDebts().cards || []).find((c) => c.acct.id === acct.id); return ce ? ce.outstanding : null; }
      return window.fhPersonalBalance ? fhPersonalBalance(acct.id) : null;
    };
    ...
    /* "Cài đặt tài khoản" — the SAME screen in settings mode (one account,
       Huỷ / Lưu, no dots, no Để sau). On an anchored account the number row
       shows today's figure; typing a new one and saving re-anchors ("Mốc
       mới"), the same declared-truth semantics as the first setup. */
    window.fhAcctEditSheet = function (acctId) {
      const P = _P(); const acct = P && P.accounts.find((a) => a.id === acctId); if (!acct) return;
      _wiz = { mode: 'settings', queue: [acctId], i: 0, intro: false, saved: 0, acct: acct, attempted: false, draft: _wzDraft(acct) };
      _wizRender();
    };
    function _wizStep(queue, i, intro, saved) {
      ...
      _wiz = { mode: 'wizard', queue: queue, i: i, intro: intro, saved: saved, acct: acct, attempted: false, draft: _wzDraft(acct) };
      _wizRender();
    }
    /* The screen: the debt overlay (sheets sit above it, like the personal
       ...
       that opens the app's own picker sheets, two buttons at the bottom.
       Re-rendered after every pick, the name read back from the field first. */
    function _wizRender() {
      const w = _wiz; if (!w) return;
      const d = w.draft, settings = w.mode === 'settings', n = w.queue.length, i = w.i, last = i === n - 1, isCard = d.kind === 'credit_card';
    ...
    window.fhWizSheet = function (which) {
      ...
      if (which === 'amt' || which === 'lim') {
        ...
        const cur = isAmt ? (d.amtSet ? d.amtK : _wzLive(w.acct)) : d.limitK;
    ...
    /* Xong / Lưu. The anchor is written only when the person typed one (the
       ...
       negative asset). One write, one hydrate. */
    window.fhWizSave = async function () {
      const w = _wiz; if (!w) return; _wizReadName();
      const d = w.draft, isCard = d.kind === 'credit_card', settings = w.mode === 'settings';
      if (!settings && !d.amtSet) { w.attempted = true; _wizRender(); window.toast && toast('Nhập số đang thấy trong app ngân hàng nhé'); return; }
      const btn = document.querySelector('#dbt-body .dbt-btn.primary'); const lbl = btn ? btn.textContent : '';
      ...
      if (settings) { _wiz = null; window.toast && toast('Đã lưu'); if (isCard) openDebtAccount(w.acct.id); else openBalAccount(w.acct.id); if (window.renderPersonal) renderPersonal(); return; }
      if (w.i + 1 < w.queue.length) _wizStep(w.queue, w.i + 1, w.intro, w.saved + 1); else _wizDone(w.saved + 1);
    };
    /* settings mode Huỷ: back to the detail the person came from */
    window.fhWizCancel = function () {
      const w = _wiz; _wiz = null; if (!w) { closeDebt(); return; }
      if (w.acct.kind === 'credit_card') openDebtAccount(w.acct.id); else openBalAccount(w.acct.id);
    };
