  ...
  function _friendly(e) {
    ...
    /* One mailbox, one reader (0103). Named BEFORE the generic duplicate-key
       line, which would otherwise answer "that already exists" — true, and
       useless: the thing that already exists is a connection on a DIFFERENT
       account, which is not something you can see from here. Say which mailbox
       and what to do, because the fix is on the other account, not this one. */
    if (/mailbox_grants_one_per_mailbox/i.test(raw))
      return L('Hộp thư này đã được kết nối ở một tài khoản khác. Ngắt kết nối ở tài khoản đó trước, rồi thử lại.',
               'This mailbox is already connected on another account. Disconnect it there first, then try again.');
