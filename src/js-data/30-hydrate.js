    ...
    try {
      ...
      if (snap) {
        ...
        const R = await Promise.all([
          ...
          sb.from('transactions').select('id,category_id,member_id,note,note_enc,amount,amount_enc,occurred_time,occurred_time_enc,txn_date,status,created_by,created_at').eq('family_id', fid).order('txn_date', { ascending: false }),
      ...
      // layer-1 data consent (0082): once per user, off the critical path, and
