  ...
  window.fhCardEnterPrompt = function () {
    _fhModal({
      ...
      body: '<div class="fh-s-sub">' + L('Dán thẻ khóa vào đây để mở dữ liệu của nhà.',
                                         'Paste your family key here to open the data.') + '</div>'
        ...
        + _btn(L('Dán thẻ khóa', 'Paste'), 'fhCardPasteInput(this)', _S.line)
  ...
  (function _fhBootCard() {
    try {
      ...
      ov.innerHTML = '<div style="max-width:420px;width:100%;text-align:center">'
        + '<div style="font-size:19px;font-weight:700;margin-bottom:12px">' + L('Đã nhận thẻ khóa của nhà', 'Got your family Key Card') + '</div>'
        + '<div style="color:var(--muted);line-height:1.6;margin-bottom:22px">' + L('Thẻ khóa đã lưu trên máy này. Chọn cách bạn muốn vào nhà.', 'The key is saved on this device. Choose how you’d like to get in.') + '</div>'
        ...
        + '<button id="fh-handoff-copy" style="' + _hbtn + ';background:none;color:var(--brand);border:1.5px solid var(--brand)">' + L('Chép thẻ khóa', 'Copy the key') + '</button>'
        + '<div style="color:var(--muted);font-size:12px;line-height:1.5;margin-top:14px">' + L('Dùng “Chép thẻ khóa” nếu bạn sẽ mở app đã cài trên máy.', 'Use “Copy the key” if you’ll open the installed app instead.') + '</div>'
        + '</div>';
      const mount = () => {
