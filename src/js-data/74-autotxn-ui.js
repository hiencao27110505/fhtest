  ...
  async function _atxConnection() {
    try {
      ...
      if (!row) return null;
      /* needsReauth, camelCased, because that is what fhAutoTxnStatus already
         reads. A token dies every 7 days while the OAuth app is in Testing, so
         this is a routine state and not an edge case. */
