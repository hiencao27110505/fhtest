  ...
  /* ── which account Google should open on ───────────────────────────────────
     THIS CAUSED A REAL ACCOUNT SPLIT. The hint was always the SIGN-IN address,
     and a person whose bank writes to a different mailbox than the one they log
     in with (the live case: login gichisreading@, mail at trang.nguyen.wh@) got
     Google's picker defaulted to the wrong account on every reconnect. Consent
     there mints a grant under a DIFFERENT auth user, and rows staged under the
     old one become invisible to the person who owns the mailbox — 60 of them,
     stranded, before this was noticed.

     So the mailbox ALREADY ON THE GRANT wins. It is the only address we know
     for certain the bank writes to, because we have been reading it.

     Order, most specific first:
       1. an address typed in this session — an explicit choice, always obeyed
       2. the mailbox on the existing grant — right for every reconnect
       3. the sign-in address — the first-connect default, and a decent guess

     Still only a `login_hint`: it picks which account Google opens on and can
     never grant anything. Whoever consents on Google's screen is who we get. */
  let _atxKnownMailbox = null;

  const _atxHintEmail = () => _atxTyped || _atxKnownMailbox || _atxLoginEmail();

  function _atxAcctRow() {
    /* Names whatever the CTA beside it will actually use, so the row can never
       promise an account the button ignores. */
    const email = _atxHintEmail();
  ...
  async function _atxConnection() {
    try {
      ...
      /* The mailbox actually on the grant, remembered for the login_hint. */
  ...
  window.fhAutoTxnGrant = async function () {
    ...
    const win = _atxOpenBlank();
    try {
      _atxNavigate(win, await _atxConsentUrl(_atxHintEmail()));
  ...
  async function _atxFrontier(gid, sinceIso) {
    /* SCOPED, when a catch-up asks. The all-time minimum is the right frontier
       for a first backfill reading into an empty queue; it is the WRONG one for
       a catch-up, where 88 rows from months ago are already pending and would
       peg the answer to the oldest of those. Passing `sinceIso` filters to the
       outage window, so the frontier walks back through the gap instead of the
       whole ledger. The localStorage floor is deliberately skipped in that
       mode: it remembers the all-time frontier, which is a different quantity. */
    const scoped = !!sinceIso;
    let floor = null;
  ...
  /* ── a catch-up is an outage being read back ───────────────────────────────
     After a reconnect the worker's own windowDays() already covers exactly the
     gap — measured from last_synced_at, which SURVIVES the reconnect because
     grant_mailbox_access does not touch it on conflict. So there is nothing to
     request and nothing to store: a healthy grant whose last sync is far older
     than the cron cadence IS mid-catch-up, and that is the whole detection.

     Threshold is 3x the five-minute tick. Tighter and an ordinary slow run
     reads as an outage; looser and a real one stays invisible for too long. */
  const ATX_CATCHUP_MIN_MS = 15 * 60 * 1000;

  function _atxCatchUpMs(conn) {
    if (!conn || conn.needsReauth || !conn.backfilledAt || !conn.lastSyncedAt) return 0;
    const gap = Date.now() - Date.parse(conn.lastSyncedAt);
    return (Number.isFinite(gap) && gap > ATX_CATCHUP_MIN_MS) ? gap : 0;
  }

  /* The gap survives its own closing, so the card can say what it covered.
     last_synced_at jumps to now the moment a run finishes the window, which
     would otherwise erase the number in the same instant it became true. */
  let _atxCatchUpSeen = 0;
  ...
  /* WIDENED DELIBERATELY (2026-09-13). It used to ask for clear columns only,
     and a test asserted that — because at the time the amount was the one thing
     this list could not show. The product call changed: a row reading
     "Highlands Coffee Nguyen Hue · −92.000đ" proves the pipeline is alive in a
     way "Techcombank" never does, and it is the same content the review screen
     already opens two taps later. So the sealed envelope comes down too and is
     opened HERE, on the client, exactly as fhReadStagedRow does it.

     The clear columns still carry the fallback: when the ledger is locked the
     row degrades to provider + date rather than rendering a padlock, which
  ...
  const ATX_FEED_ROWH = 58;          // must match .atx-fd-row height in CSS
  ...
  function _atxFeedRowHTML(r) {
    ...
    /* Opened: the description leads, because that is what a person recognises,
       and the bank becomes provenance. Locked: the provider leads and the row
       is exactly what shipped before — a degrade, never a padlock. */
    const lead = r._desc || name;
    const sub = r._desc ? (when ? name + ' · ' + when : name) : when;
    /* Staged amounts are FULL VND; fmt() multiplies by curMult(), so the divide
       is what keeps this from rendering a thousand times too large. */
    let amt = '';
    if (r._amount != null && window.fmt) {
      const mult = window.curMult ? window.curMult() : 1;
      const n = Math.abs(Number(r._amount) || 0) / mult;
      amt = (r._direction === 'credit' ? '+' : '−') + window.fmt(n);
    }
    return '<div class="atx-fd-row"><span class="atx-fd-ic">' + _esc(_atxInitials(name)) + '</span>' +
      '<span class="atx-fd-b"><span class="atx-fd-t">' + _esc(lead) + '</span>' +
      (sub ? '<span class="atx-fd-s">' + _esc(sub) + '</span>' : '') + '</span>' +
      (amt ? '<span class="atx-fd-a' + (r._direction === 'credit' ? ' in' : '') + '">' + _esc(amt) + '</span>'
           : '<span class="atx-fd-d">' + _esc(when) + '</span>') +
      '</div>';
  }
  ...
  function _atxProgressSub(st) {
    ...
    /* A catch-up that finished repeats the guarantee the reconnect screen made,
       word for word. That screen wrote the cheque; this is it clearing. */
    if (st.phase === 'caughtup') {
      return st.found > 0
        ? _esc(L('Đã đọc lại đủ ' + fmtGap(st.gapMs) + ' · tìm được ', 'Read back all ' + fmtGap(st.gapMs) + ' · ')) + cnt +
          _esc(L(' khoản. Không trùng, không thiếu.', ' found. No duplicates, nothing missing.'))
        : _esc(L('Không có khoản nào trong ' + fmtGap(st.gapMs) + ' bị ngắt.',
                 'No transactions in the ' + fmtGap(st.gapMs) + ' that was missed.'));
    }
    if (st.phase === 'done') {
      return st.found > 0
  ...
  function _atxProgressHTML(st) {
    ...
    const finished = st.phase === 'done' || st.phase === 'caughtup';
    const pct = finished ? 100 : (w > 0 ? Math.min(100, Math.round(d / w * 100)) : 0);
    const title = st.phase === 'done'     ? L('Đã đọc xong ' + w + ' ngày qua', 'All ' + w + ' days read')
                : st.phase === 'caughtup' ? L('Đã đọc bù xong', 'All caught up')
                : st.phase === 'catchup'  ? L('Đang đọc bù ' + fmtGap(st.gapMs), 'Catching up ' + fmtGap(st.gapMs))
    ...
    const fillCls = finished ? ' done' : (st.phase === 'slow' ? ' halt' : '');
    return '<div class="atx-pg' + (finished ? ' done' : '') + '" data-phase="' + st.phase + '">' +
  ...
  async function _atxProgressState(conn) {
    const catchUp = _atxCatchUpMs(conn);
    if (catchUp) _atxCatchUpSeen = catchUp;
    /* A catch-up that just closed still has a story to tell, once. */
    const closing = !catchUp && _atxCatchUpSeen > 0 ? _atxCatchUpSeen : 0;
    const windowDays = catchUp
      ? Math.max(1, Math.ceil(catchUp / 86400000))
      : ((conn && conn.backfillDays) || ATX_DEFAULT_DAYS);
    let found = 0, front = null, finds = [];
    try { found = await _atxPendingCount(); } catch (e) {}
    if (conn && conn.id) {
      try { front = await _atxFrontier(conn.id, catchUp ? conn.lastSyncedAt : null); } catch (e) {}
    }
    const phase = catchUp ? 'catchup'
                : closing ? 'caughtup'
                : ((conn && conn.phase) || 'reading');
    if (found > 0 && (phase === 'reading' || phase === 'catchup')) finds = await _atxRecentFinds();
    if (closing) _atxCatchUpSeen = 0;            // said once, then forgotten
    _atxProgressCache = { phase: phase, windowDays: windowDays,
             found: found, front: front, finds: finds,
             gapMs: catchUp || closing,
