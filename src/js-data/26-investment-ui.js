  ...
  (function () {
    ...
    async function _logoSweep(rows) {
      ...
      if (dirty) { try { localStorage.setItem(_LK, JSON.stringify(all)); } catch (e) {} _redraw(); _reopen(); }
    }
    /* the position's face: local logo bytes if the coin is top-30, else a
       class mark, so vàng/CK/CCQ read at a glance too */
    const CLS_MARK = { crypto: '🪙', gold: '🥇', stock: '📊', fund: '🧺', other: '💼' };
    const _logoHtml = function (p) {
      const uri = (p.klass === 'crypto' && p.symbol) ? _logoAll()[String(p.symbol).toLowerCase()] : null;
      if (uri) return '<img class="inv-logo" src="' + uri + '" alt="">';
