  ...
  var FH_CONSENT_CHANGES = {
    bank_email: {
      4: ['Với luồng đọc trực tiếp hộp thư, tên người nhận và số tài khoản hiện được gửi nguyên văn cho AI, không còn được che như trước. Số tiền và địa chỉ email thì vẫn luôn được che.',
          'On the direct mailbox-read path, counterparty names and account numbers are now sent to the AI as written, no longer masked as before. Amounts and email addresses are still always masked.'],
    },
    app_data: {},
  };
    ...
    window.fhConsentSheet = async function (opts) {
      ...
      var rows =
        _cstRow(L('Có ai đọc được email của tôi không?', 'Can anyone read my emails?'), _esc(L(
          'Email được xử lý tự động rồi tự xoá sau 7 ngày; email bị lỗi giữ tối đa 90 ngày rồi cũng xoá. Khi AI cần đọc cấu trúc email, số tiền và địa chỉ email luôn được thay bằng ký hiệu. Với email bạn chuyển tiếp, tên và số tài khoản cũng được che. Với luồng đọc trực tiếp hộp thư, phần chữ còn lại, ví dụ tên người nhận, được gửi nguyên văn.',
          'Emails are processed automatically and delete themselves after 7 days; ones we fail to read are kept at most 90 days, then deleted too. When the AI reads an email’s structure, amounts and email addresses are always replaced with placeholders. For email you forward, names and account numbers are masked too. On the direct mailbox-read path, the remaining text, such as a counterparty’s name, is sent as written.'))) +
