  ...
  (function () {
    ...
    /* The full bank-email consent, as one embeddable block: kicker, the
       what-changed delta for a re-consent, the Q&A body (folded behind the
       delta on a re-consent, in full for a first ask) and the small print.
       Built for the connect flow's step 1 (activation-journey-spec Q28): the
       consent is read and agreed INSIDE the flow, one continuous read → agree
       → Google, instead of a second sheet interrupting it. The standalone
       fhConsentSheet below stays for every other caller (review-entry offer,
    ...
    /* {needed, prior}: is a bank-email consent at the current version on
       ...
       the silence costs a lawful basis. */
    window.fhConsentState = async function () {
      var rec = null;
      try { rec = await _cstFetch(); } catch (e) { return { needed: true, prior: null }; }
      return { needed: !(rec && rec.version >= FH_CONSENT_V), prior: rec };
    };
    /* Record the agree (same row fhConsentAgree writes). True = recorded (or
       already there); false = the insert failed and the caller must not proceed
       as if consent existed. */
    window.fhConsentRecord = async function () {
      var res = await sb.from('user_consents').insert({ kind: FH_CONSENT_KIND, version: FH_CONSENT_V });
      var dup = res.error && /duplicate key|already exists/i.test(res.error.message || '');
      if (res.error && !dup) return false;
    ...
    window.fhConsentSheet = async function (opts) {
      ...
      var rows = _cstBankRows();
      var smallPrint = _cstBankSmallPrint();
      var footer;
      if (ro) {
        var when = accepted && accepted.consented_at ? new Date(accepted.consented_at) : null;
        footer = smallPrint +
    ...
    function _cstBankSmallPrint() {
      return '<div class="cst-meta">' + _esc(L(
