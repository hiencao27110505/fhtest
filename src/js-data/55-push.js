  ...
  /* Silent re-sync, once per app session after the first hydrate: endpoints
     rotate and the active family can change, so an opted-in device re-points
     its row at whatever family it wakes up in. */
  window.fhPushResync = async function () {
    try {
      if (window._fhPushSynced) return; window._fhPushSynced = true;
      if (!_pushSupported() || Notification.permission !== 'granted') return;
      const sub = await _pushGetSub(); if (!sub) return;
      await _pushSaveSub(sub);
    } catch (e) {}
  };
