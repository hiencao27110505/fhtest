  ...
  window.fhPushSheet = async function () {
    ...
    if (st === 'unsupported') {
      ...
      sub = document.documentElement.classList.contains('fh-nofam')
        ? L('Máy này sẽ nhận thông báo khi có giao dịch từ email chờ bạn duyệt, kể cả khi app đang đóng.', 'This device gets a heads-up when a bank email is waiting for your review, even with the app closed.')
      ...
      sub = document.documentElement.classList.contains('fh-nofam')
        ? L('Nhận thông báo khi có giao dịch từ email chờ bạn duyệt. Kể cả khi app đang đóng.', 'Get a heads-up when a bank email is waiting for your review. Even with the app closed.')
        : L('Nhận thông báo khi cả nhà ghi một khoản, thêm ảnh, thả cảm xúc, gửi yêu cầu hoặc chia sẻ tâm trạng, và khi có giao dịch từ email chờ bạn duyệt. Kể cả khi app đang đóng.', 'Get a heads-up when the family logs an expense, adds a photo, reacts, sends a request or shares a mood, and when a bank email is waiting for your review. Even with the app closed.');
  ...
  window.fhPushFirstVisitOffer = function () {
    try {
      /* A famless account has no member seat — the person themselves is the
         once-per key then (0152). */
      const mid = (window.DB && window.DB.ownerMemberId) || (window.fhUser && window.fhUser.id); if (!mid) return;
