...
/* The provider is compared through the shared identity fold since 2026-09-23
   (account-identity-fold.test.js), so the match reads `akey(a) === pkey`; the
   thing pinned here is unchanged — provider only, never the kind. */
t('a tail-less ensure() matches by provider, never by kind',
  /: \(akey\(a\) === pkey && !a\.tail\);/.test(data) && !/a\.kind === info\.kind && akey\(a\) === pkey && !a\.tail/.test(data));
