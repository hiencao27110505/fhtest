...
(async () => {
  ...
  t('the file opened: sealed details, hash check, table read', /Tìm được 13 giao dịch/.test(html), html.slice(0, 160));
  t('failed rows are counted, not shown', /1 thất bại đã bỏ qua/.test(html) && (html.match(/class="stm-prow/g) || []).length === 13);
  ...
  t('a payment funded by a bank says so', /trả từ ngân hàng liên kết/.test(html));
  t('nothing has been written yet', calls.rpc.length === 0 && /Chưa có gì được ghi vào sổ/.test(html));
