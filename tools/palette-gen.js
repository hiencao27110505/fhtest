...
/* Palette generator — the single place FamilyHub colours are COMPUTED (DESIGN.md §2.1).
   ...
   The scheme (see DESIGN.md §2.1 "The harmony scheme"):
     ...
     danger      red H15  — crimson, 20° warm of the exact complement (355) so red and sage never
                 vibrate side by side; one step louder than the other two states
...
/* Status ramps. Amber weighs the same as sage (equal-weight rule). Danger is the one state allowed a
   step LOUDER — its job is to interrupt — so red carries more chroma at every step. Hue 15 = crimson:
   20° warm of sage's exact complement (355), far enough never to vibrate beside it, close enough to
   keep the contrast; chosen from six candidates on 2026-09-14. */
const STATUS = {
  red:   { H: 15, tint: [.96, .03],  light: [.78, .13], base: [.58, .20], ink: [.47, .17] },
  amber: { H: 70, tint: [.96, .025], light: [.78, .11], base: [.62, .15], ink: [.50, .13] },
};
const GUIDE = { warn: 85, hot: 50 };                      // ok = the sage ramp; over = the red ramp (below)
...
for (const name in STATUS) for (const step of ['tint', 'light', 'base', 'ink']) out[name][step] = hex(STATUS[name][step][0], STATUS[name][step][1], STATUS[name].H);
...
out.guide.over = { main: out.red.base, mut: hex(GUIDE_SHAPE.mut[0], GUIDE_SHAPE.mut[1], STATUS.red.H), bg: out.red.tint };   // over = the red ramp, label tone at the red hue
...
for (const name of ['red', 'amber']) lines.push('  ' + ['tint', 'light', 'base', 'ink'].map((s) => `--${name}-${s}:${out[name][s]};`).join(' '));
