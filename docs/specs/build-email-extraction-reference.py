# Build email-extraction-reference.docx from email-extraction-reference.md
#
# Generator reused verbatim from build-personal-ledger-spec.py — same Calibri /
# #35654D house style, same real Heading 1/2/3 so Word's navigation pane works.
# Only the source, output and title block differ: two docs in one set should not
# be two typographies, and a second implementation of the same style is a second
# thing to drift.
# Style: matches bank-email-capture-spec.docx (Calibri, green #35654D accents)
# but with REAL Heading 1/2/3 styles so Word's navigation pane works.
import re, os
...
SRC = os.path.join(os.path.dirname(os.path.abspath(__file__)), "email-extraction-reference.md")
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "email-extraction-reference.docx")
...
def _style_heading(name, size, color, bold=True, before=18, after=6, caps=False):
    h = doc.styles[name]
    h.font.name = "Calibri"; h.font.size = Pt(size); h.font.bold = bold
    h.font.color.rgb = color
    h.font.all_caps = caps
    # kill the Word-default italic/kerning inheritance quirks
    h.font.italic = False
    h.paragraph_format.space_before = Pt(before)
    h.paragraph_format.space_after = Pt(after)
    h.paragraph_format.keep_with_next = True
    # headings must not inherit the blue theme color via rPr theme refs
...
def runs_from_inline(p, text, base_size=None, base_color=None):
    """Parse **bold**, *italic*, `code` (nesting: code inside bold) into runs."""
...
# ── title block (direct formatting, like the reference doc) ────────────────
...
i = 0
first_h1_done = False
# lead paragraph after the doc title becomes the subtitle zone
title_block("Email Extraction Reference",
            "How a bank email becomes fields: the three tiers, the field map, and what changed.")

# skip the md H1 title line; we re-render intro paragraphs as normal text
while i < len(lines) and not lines[i].startswith("# "):
    i += 1
i += 1  # past the markdown H1
...
cp.title = "Personal Ledger & the Cá nhân Tab — Product Specification"
cp.author = "FamilyHub"
cp.comments = ""
try:
