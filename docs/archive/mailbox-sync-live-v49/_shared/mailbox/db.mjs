...
export function createDb(url, serviceKey, fetchImpl) {
  ...
  return {
     ...
     *
     * Plural since 0136: a mailbox may have more than one reader (two accounts
    ...
    async stagedState(messageIds, memberId, ownerUserId) {
      ...
      for (let i = 0; i < messageIds.length; i += CHUNK) {
        ...
        /* SCOPED TO THE READER (0136), like the tombstones below. Unscoped, this
     ...
     * family (0136: one household inbox, two readers). Oldest first, so every
     ...
     *
     * A unique violation on `(owner_user_id, gmail_message_id)` (0136) returns
