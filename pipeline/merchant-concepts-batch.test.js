  ...
  return { store, subtle, llm: { apiKey: opts.noKey ? null : 'k' },
    ...
    fetch: async (u, init) => {
      ...
      const items = asked.split('\n').map((line) => {
        ...
        return { i, concept: /BARBER/.test(line) ? 'Others' : /SUKIYA/.test(line) ? 'Dining' : null, pool: null };
      });
...
ctx = makeCtx({});
out = await C.conceptsForMerchants(['SUKIYA', 'DONG TAY BARBER', 'ZZ OPAQUE 1', 'SUKIYA', '  '], 'u1', ctx);
...
t('answers mapped back by position', out.concepts['SUKIYA'] === 'Dining' && out.concepts['DONG TAY BARBER'] === 'Others' && out.concepts['ZZ OPAQUE 1'] === null, out.concepts);
...
var again = await C.conceptsForMerchants(['SUKIYA', 'ZZ OPAQUE 1'], 'u2', ctx);
t('a second user asking the same merchants costs nothing', ctx.store.calls.length === 1 && again.concepts['SUKIYA'] === 'Dining');

console.log('\n-- a limit slows, never poisons --');
ctx = makeCtx({ limited: true });
out = await C.conceptsForMerchants(['SUKIYA', 'DONG TAY BARBER'], 'u1', ctx);
t('rate-limited: nulls and limited=true', out.limited === true && out.concepts['SUKIYA'] === null);
t('...and NOTHING cached, so the next statement asks again', ctx.store.puts.length === 0);
ctx = makeCtx({ noKey: true });
out = await C.conceptsForMerchants(['SUKIYA'], 'u1', ctx);
