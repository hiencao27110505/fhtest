...
- **Follow-ups the same day (v515–v517).** No autofocus on open (iOS scrolled the modal off-screen); tail-less ensure() matches by provider, never kind, plus a once-per-session heal folding existing tail-less twins; the wizard rebuilt as a rows screen in the debt overlay (name the one input, picker sheets for the rest, calendar pickers for the two days); and the old "Cài đặt tài khoản" modal folded into that screen as settings mode (Huỷ / Lưu, re-anchor by typing a new number). Then (v518) every date or time settings row in the app became a picker row (`fhPickRow`, 12-format-helpers): a transparent OS input covers the row so the tap opens the calendar or clock itself, no sheet in between — the wizard's two days, the transaction details' Ngày / Giờ, the review card's Ngày / Giờ and Hẹn trả. The `sheet-exd-when` sheet and the review's when/loandue sheet branches are gone.
